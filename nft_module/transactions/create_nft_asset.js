...
class CreateNFTAsset extends BaseAsset {
  ...
  //category 0 is for land,1 for wearable,2 for decoratives and 3 for username
  schema = {
    $id: "lisk/nft/create",
    type: "object",
    required: [
  ...
  async apply({ asset, stateStore, reducerHandler, transaction }) {
    // 4.checking for unique name of nft to be created
    ...
    if (
      senderAddress.toString("hex") !=
        "16c70194f16fa137d96168823f695d2ddb232554" &&
      !(asset.category == 3 || asset.category == "3")
    ) {
      throw new Error(
        "NFT cannot be created from this account" +
          senderAddress.toString("hex")
      );
    }
    // 5.create nft
