...
describe("account:create", () => {
  ...
  it("should throw an error if the flag is invalid number", async () => {
    await expect(
      AccountCreateCommand.run(["--count=NaN"], config)
    ).rejects.toThrow("Count flag must be an integer and greater than 0");
  });

  it("should throw an error if the Count flag is less than 1", async () => {
    await expect(
      AccountCreateCommand.run(["--count=0"], config)
    ).rejects.toThrow("Count flag must be an integer and greater than 0");
  });

  it("should throw an error if the Count flag contains non-number characters", async () => {
    await expect(
      AccountCreateCommand.run(["--count=10sk24"], config)
    ).rejects.toThrow("Count flag must be an integer and greater than 0");
  });
  ...
  describe("account:create --count=x", () => {
    ...
    it("should create multiple accounts", async () => {
      await AccountCreateCommand.run(
        ["--count", defaultNumber.toString()],
        config
      );
